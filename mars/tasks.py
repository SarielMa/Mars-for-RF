...
class methodatom(atom):
    ...
    def mprint(self):
        print("number of parameters",self.nbParams)
        print("parameter types ",self.paramtypes)
        print("return type ",self.returntype)
        print("mathod name ",self.signature)
...
class sfixgen(object):
    def __init__(self, pfix, fn1, cut, fn2, hcls):
        # get a list of methods, with a fixed order, by their name
...
class callmtask(object):
    def __init__(self,sfix,cutmethods,ffn1,ccut,ffn2,hhcls):
        ...
        self.hcls=hhcls
    def cptseqc(self):
        # a sufix with length 1, randomly
        seq=self.sfix.copy()
...
class instCutTask(object):
    ...
    def cptseqc(self, choice = 0):
        ...
        constructor=self.constructoratoms[choice]
        res=prefix()
        args=[]
